...
public class ST_PuzzleDisplay : MonoBehaviour 
{
	...
	private ST_PuzzleTile[,] TileDisplayArray; ///////// 
	...
	void Start () 
	{
		...
		// mix up the puzzle. // タイマー導入に向けて削除
		//StartCoroutine(JugglePuzzle()); // タイマー導入に向けて削除
	}
	...
	private ST_PuzzleTile GetTileAtThisGridLocation(int x, int y, ST_PuzzleTile thisTile)
	{
		for(int j = Height - 1; j >= 0; j--)
		{
			for(int i = 0; i < Width; i++)
			{
				...
				{
					if(TileDisplayArray[i,j].Active == false)
					{
	...
	// タイマー導入に向けて追加 Dec10/2025
	public IEnumerator StartShuffle()
	{
    	// 既存のJugglePuzzleと同じロジックを使用
    	yield return StartCoroutine(JugglePuzzle()); 
	}
	...
	private IEnumerator JugglePuzzle()
	{
		...
		TileDisplayArray[0,0].Active = false;
		...
		for(int k = 0; k < 20; k++)
		{
			...
			for(int j = 0; j < Height; j++)
			{
				...
				{		
					...
					TileDisplayArray[i,j].ExecuteAdditionalMove();
		...
		// continually check for the correct answer. // タイマー導入に向けて削除
		// StartCoroutine(CheckForComplete()); // タイマー導入に向けて削除
	...
	public IEnumerator CheckForComplete()
	{
		while(Complete == false)
		{
			...
			for(int j = Height - 1; j >= 0; j--)
			{
				...
				{
					// check if this tile has the correct grid display location.
					if(TileDisplayArray[i,j].CorrectLocation == false)  
					{
						Complete = false;
					}
				}
		...
		if(Complete)
		{
            ...
            for (int j = 0; j < Height; j++)
            {
                for (int i = 0; i < Width; i++)
                {
                    if (TileDisplayArray[i, j].Active == false)
                    {
                        TileDisplayArray[i, j].GetComponent<Renderer>().enabled = true;
                        TileDisplayArray[i, j].GetComponent<Collider>().enabled = true;
                        TileDisplayArray[i, j].Active = true;
                        break;
                    }
                }
            }
	...
	private void CreatePuzzleTiles()
	{
		...
		TileDisplayArray = new ST_PuzzleTile[Width,Height];
		...
		for(int j = Height - 1; j >= 0; j--)
		{
			for(int i = 0; i < Width; i++)
			{
				...
				GameObject spawnedTile = Instantiate(Tile, new Vector3(0.0f, 0.0f, 0.0f) , Quaternion.Euler(90.0f, -180.0f, 0.0f)) as GameObject;
				spawnedTile.gameObject.transform.parent = this.transform;

				// set and increment the display number counter.
				ST_PuzzleTile thisTile = spawnedTile.GetComponent<ST_PuzzleTile>();
				TileDisplayArray[i, j] = thisTile; // ST_PuzzleTile コンポーネントの参照を直接格納
